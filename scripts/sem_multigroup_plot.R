...
sem_lr_free <- sem(model = cfa_lr_cond,
...
estimates_95 <- parameterEstimates(sem_lr_free,level = 0.95) %>% mutate(group = case_when(
  group == 1 ~ "Right_Multinationals",
  group == 2 ~ "Center_Multinationals",
  group == 3 ~ "Left_Multinationals",
  group == 4 ~ "Center_Immigrants",
  group == 5 ~ "Left_Immigrants",
  group == 6 ~ "Right_Immigrants"
))
estimates_90 <- parameterEstimates(sem_lr_free,level = 0.90) %>% mutate(group = case_when(
  group == 1 ~ "Right_Multinationals",
  group == 2 ~ "Center_Multinationals",
  group == 3 ~ "Left_Multinationals",
  group == 4 ~ "Center_Immigrants",
...
ggsave("figures/sem_expq80_multigroup.png", width = 20, height = 10, units = "cm") 
